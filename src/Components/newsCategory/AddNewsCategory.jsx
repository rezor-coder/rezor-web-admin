...
function AddNewsCategory() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const API_URL = id
      ? `${API_BASE_URL}/V1/updateNewsCategory/${id}`
      : `${API_BASE_URL}/V1/addNewsCategory`;
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`${id ? "Category updated" : "Category created"} successfully.`); navigate("/newsCategory");
      } else {
      ...
      if (id) {
        try {
          const response = await fetch(
            `${API_BASE_URL}/V1/public/getByNewsCategoryId/${id}`,
            {
              withCredentials: true,
              credentials: 'include',
              headers: {
